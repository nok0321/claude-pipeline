...
>
> **注 (Pipeline v2 / 2026-06-07)**: goal 駆動リデザインに着手 ([plans/PIPELINE-V2-PLAN.md](plans/PIPELINE-V2-PLAN.md))。`task-planner` (計画フェーズ) / `ship` (git デリバリ) / `skill-authoring` (skill 著作) の 3 skill と `tech-comparator` subagent を追加。上記 2026-05-23 ノートの「現在 7 skill」を更新し **現在 10 skill 構造**。design-phase に `--reverse` (実装→DESIGN 後追い)、impl-orchestrator に goal mode (DESIGN/*.md または SHARED-CONTRACT.md+Tasks を入力) を追加。前進自動化の Stop hook (`stop-ship-suggest.sh`) を追加 ([docs/event-automation.md](docs/event-automation.md))。
...
[Layer 1: フェーズオーケストレーター]
  task-planner           ─── goal 駆動の計画フェーズ専任 (技術選定→計画→共有契約→計画レビュー)
  design-phase           ─── 設計フェーズ専任 (--reverse で実装→DESIGN 後追い生成)
...
[Layer 3: ユーティリティ]
  ...
  ship                   ─── git デリバリ (commit / PR / 任意で merge、branch 保護を検出)
...
[Layer 4: Technical-judgment subagents (agents/)]
  ...
  tech-comparator        ─── 技術選定肢の多軸比較 → ランク付け (read-only)
```
...
似た名前のスキルが多いため、選択指針を一覧化する (Pipeline v2 以降の現行構成、計 10 skill + technical-judgment subagents)。

| やりたいこと | 使うスキル | 補足 |
|-------------|-----------|------|
| goal をタスク計画に分解 (技術選定 + 計画レビュー) | `task-planner` | goal 駆動フロントエンド。出力は順序付きタスク + SHARED-CONTRACT.md |
| 設計書から実装まで自律化 | `impl-orchestrator` | エントリーポイント兼任。DESIGN/*.md 不在時は design-phase、goal のみなら task-planner へフォールバック |
...
| 技術選定肢の多軸比較 → ランク付け | `tech-comparator` subagent | `agents/tech-comparator.md` (task-planner Step 2) |
| 変更を commit / PR / (任意)merge | `ship` | branch 保護を検出して PR フロー、merge は既定でユーザーゲート (`--merge` opt-in) |
| 長時間セッションの区切り | `checkpoint` | `/clear` 前 |
| repo に house-style 準拠の skill を新設/強化 | `skill-authoring` | skill-creator ラッパ＋ARCHITECTURE/README/eval 登録 |
...
### 11.3 確認負荷の計測と §A 進行確認の原則 (2026-06-07)

Pipeline v2 着手時、ユーザーから「実プロジェクト (osi-stack-ref) で計画提示・案確認のユーザー確認が多い」と提起。確認削減が v2 で達成されたかを、osi-reference の実セッション transcript (11 主要セッション) を 2 subagent で分類計測。

**結果**: 確認ポイント ~18 件のうち **C=儀式/過剰確認 ~70%** (推奨提示後の「進めてよいか」rubber-stamp、可逆な feature ブランチ作業の許可取り)、A=意図/不可逆 ~17%、**B=技術判定 ~0%** (inline 並列レビューが吸収済み)。plan mode / AskUserQuestion / handoff の実使用はいずれもゼロ (継続は CHECKPOINT.md ＋手動プロンプト、確認はテキスト会話)。

**含意**: v2 の確認削減レバー (task-planner / tech-comparator / plan-review) は B 狙いだが B は既にゼロ＝ターゲットずれ。削るべきは C で、新スキルではなく「可逆作業は聞かない」を default 化する設定。→ §A に **「進行確認の原則」** を追加 (儀式確認は Tier ではない／可逆作業は自動進行)、impl-orchestrator と ship に反映。`/ship` の git 儀式バンドル (commit?→push?→PR?) も C の一部を削る。

**未対応 (監視/任意)**: named skill の発火率が低い (ループを plain Claude で実行しがち)、background-agent の無言ストール (確認に見えた friction の一部)、osi-stack-ref 側 CLAUDE.md override、handoff の checkpoint 統合。
...
### 進行確認の原則 — 可逆作業は自動進行 (儀式確認は Tier ではない)

§A の Tier は **Finding (直す/直さない)** の分類である。これとは別に、**ワークフローの進行**について全 skill が従う原則 (Pipeline v2 / §11.3 計測で追加):

- **既に承認されたスコープ/デリバリ内の、可逆かつ git 管理下の作業は、各ステップを確認せず自動進行する。** local 編集・commit、および承認済みデリバリ (例: ユーザーが `/ship` を起動した後の commit→push→PR 作成) の構成ステップを、推奨提示後に「進めてよいか」と rubber-stamp で聞き直さない。「どの順で」「PR を分割するか」も自分で決める。
- **確認するのは次のみ:**
  1. 破壊的/不可逆 — `push --force` / history rewrite / `reset --hard` / バックアップ無し削除
  2. **未依頼の**外向き操作の開始 — 依頼されていないのに push / PR / publish / Slack 送信を始める、main への merge
  3. スコープ外・意図判定 — Tier 1 (設計選択・要件・業務判断)
- 判断は「可逆か？ 既に承認された範囲か？」。両方 Yes → 進める。不可逆 or 未承認の外向き → 確認。

これは collaboration preference の batch-when-reversible を構造化したもの。同 preference の「push / PR は外部影響＝確認」は **未依頼の自律開始**に適用し、ユーザーがデリバリを承認した後 (/ship 起動・スコープ承認後) の構成ステップには適用しない (その区別が無いと rubber-stamp 儀式になる)。根拠は §11.3。
...
Phase 2 で `pipeline-state` skill を本ドキュメントに吸収。`PIPELINE-STATE.md` のフォーマットと管理ルールを定義し、`impl-orchestrator` が phase 遷移時に、`task-planner` が Tasks / Shared contract を (goal 駆動モードで) 更新する。
...
## Tasks
(goal-driven mode — filled by task-planner; columns: # | Task | Summary | Depends on | Tech decisions | Status)

## Shared contract
(goal-driven mode — SHARED-CONTRACT.md summary: <n> types / <n> APIs / <n> tables / <n> invariants)

## Design artifacts
(design-first mode — fill in during design)
...
|---------|---------|
| `plan` | 計画サマリーの記入・改訂 |
| `tasks` | (goal 駆動) タスク表の記入・状態更新 — task-planner が記入、impl-orchestrator が status 更新 |
| `shared_contract` | (goal 駆動) SHARED-CONTRACT.md サマリの記入・更新 |
...
許可される前進: `planning → design → implementation → testing → reporting`。逆遷移は silent regression を生むため拒否する。**goal 駆動モードでは design を後置/省略**でき、`planning → implementation → testing → reporting` も許可する (DESIGN は impl 後の `design-phase --reverse` 任意ステップ、または生成しない)。
