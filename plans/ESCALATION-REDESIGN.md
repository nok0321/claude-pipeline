...
- [x] `.claude/settings.json` に Stop hook (drift gate) を追加。sonnet 4.6 で `Compare recent edits against DESIGN/*.md. Return JSON: {"compliant": bool, "gap": string|null}` を 15s timeout で実行
- [x] 警告のみ（user 決定はしない、`decision: block` は出さない）。判定結果は dogfooding 期間中の品質シグナルとして観察
