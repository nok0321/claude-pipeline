...
Opus 4.7 で skill の自律発火が増えたタイミングで、既存15個の使用実績と Anthropic 公式ベストプラクティスとの差分を測定。実測値（2026-04-28 集計）では **15個中7個が直近30日0回呼び出し**、最頻出は spec-audit (26回) で全体の58%を占める一極集中。設計の頂点である `dev-pipeline` (メタオーケストレーター) は0回で、`impl-orchestrator` を直接呼ぶ運用が定着している。
...
- 15個の現行 skill それぞれに triggerable evals を作成
...
- migration table: 旧15個 → 新7-8個の対応表を `docs/MIGRATION.md` として作成
- main にマージ
...
| R5 | eval 作成コスト (15 skill × 平均17件 ≈ 250件) | Claude が下書き → ユーザーレビューで進める。escalation/pipeline-state は10件、他13 skill は20件で運用 |
...
- Phase 毎に commit を切り、commit message に Phase 番号を含める（例: `phase 1: english migration of 15 skills`）
...
- [ ] 既存 15 skill の現状 SKILL.md 行数を計測（Phase 3 の改善基準値）
