...
| `spec-audit` | 設計書間矛盾検出 | geo-route-studio固有 | **要** |
| `spec-check` | 設計↔実装差分検出 | geo-route-studio固有 | **要** |
| `spec-fix` | 双方向自動修正 | geo-route-studio固有 | **要** |
| `robust-review` | セキュリティ/堅牢性深層レビュー | geo-route-studio固有 | **要** |
| `robust-fix` | S-Critical/S-High自動修正 | geo-route-studio固有 | **要** |
...
- `robust-review`: `crates/backend/src/`, `frontend/src/` 等のパスが直書き
- `spec-check`: コンポーネント→仕様書→実装ディレクトリのマッピングテーブルが固定
- 両スキルに `SurrealQL`, `no_std`, `wasm-bindgen`, `Axum` 固有のチェックが埋め込み
...
| frontend | DESIGN/06_frontend.md | frontend/src/ |

## Project-Specific Checks（CLAUDE.md に追加するセクション）
- no_std: crates/geo_core/ → std:: import 禁止、数学関数は libm 使用
- 座標系: geo_core=[lat,lon], SurrealDB=[lon,lat], Leaflet=[lat,lon]
- Axum: ハンドラ引数順 State→Path→Json
```
...
**Sprint 1 検証**:
1. geo-route-studio で小さな機能（例: 既存APIにバリデーション追加）を `/impl-orchestrator api` で実行
2. 確認項目:
...
## Sprint 2: 境界契約テ���ト生成 ✅ 完了（2026-04-12）

**目標**: API型不一致・座標変換ミスなど���insights最大フリクション(54件中の主要因)の機械的��出
...
| 座標変換境界 | CLAUDE.md `## Critical Constraints` | Grep: 変換関数群 | ラウンドトリップ(値→変換→逆変換→一致) |
...
**検証**:
1. geo-route-studio で `/boundary-test detect` → 境界一覧が正しく検出される
